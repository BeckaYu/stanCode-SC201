...
def increment(d1, scale, d2):
    ...
    """
    for token, weight in d2.items():
        # Scales each value in d2 by 'scale' and adds it to the corresponding element in d1
        d1[token] = d1.get(token, 0) + d2.get(token) * scale
...
def dotProduct(d1, d2):
    ...
    """
    if len(d1) < len(d2):
        # Swap d1 and d2 if d1 is smaller to optimize the loop for the smaller dictionary
        return dotProduct(d2, d1)
    else:
        # Calculate the dot product by multiplying corresponding values for shared keys
        return sum(d1[token] * d2[token] for token in d2 if token in d1)
